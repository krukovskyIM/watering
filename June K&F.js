...
IRsensor.on('receive', function(code) {
  ...
  // Мастер Катя
      ...
      if (code == 378132519) // столбец 3, ряд 1
          { Status = 'ON'; // включаем полив
            str1 = 'Water';
            str2 = 'is ON'; 
            IRsensorButton = 1;
          }
      if (code == 378110079) // столбец 3, ряд 2
          { Status = 'OFF'; // выключаем полив
            str1 = 'Water';
            str2 = 'is OFF'; 
            IRsensorButton = 1;
          }
      if (code == 378093759) // столбец 3, ряд 3
          { if (MoistSet < 99)
            {MoistSet++; // увеличиваем уставку влажности
            }
            str1 = 'MoistSet:';
      ...
      if (code == 378099879) // столбец 3, ряд 4
          { if (MoistSet > 1)
            {MoistSet--; // уменьшаем уставку влажности
            }
            str1 = 'MoistSet';
      ...
      if (code == 378116199) // столбец 3, ряд 5
          { if (MaxVol < 99)
            {MaxVol++; // увеличиваем суточный объем
            }
            str1 = 'MaxVol:';
      ...
      if (code == 378085599) // столбец 1, ряд 6
          { if (MaxVol > 2)
            {MaxVol--; // уменьшаем суточный объем
            }
            str1 = 'MaxVol';
...
setInterval(function() {
      ...
      {
        Screen.clear(reset); // http://www.espruino.com/Reference#l_Graphics_clear
        Screen.setFontVector(18); // выбираем размер шрифта
        Screen.drawString(str1, 0, 0); // записываем строку в буфер дисплея
        Screen.drawString(str2, 0, 20); // записываем строку в буфер дисплея
        Screen.drawString(str3, 0, 40); // записываем строку в буфер дисплея
        Screen.drawString(str4, 0, 60); // записываем строку в буфер дисплея
        Screen.flip(); // отображаем содержимое буфера на экране
      }
